...
def optimize_on_joints(j2d,
    ...
    for o_id, orient in enumerate(orientations):
        ...
        # 15: right knee,  90deg bend at np.pi/2python fit_3d.py female ../ --out_dir=./pkl
        alpha = 10
...
def run_single_fit(img,
    ...
    for deg in do_degrees:
        ...
        # now render
        im = (render_model(
            out_path.replace('.pkl', '.obj'),verts, model.f, w, h, cam, far=20 + dist) * 255.).astype('uint8')
        images.append(im)
...
def main(gender,
    ...
    if not use_neutral:
        ...
        with open(join(data_dir, 'lsp/lsp_gender.csv')) as f:
            genders = f.readlines()
    ...
    # Load images
    imgDir_path='../images/'
    img_paths=listdir(imgDir_path)
    img_paths.sort(key=lambda x:int(re.findall(r"image(.+?).png",x)[0])) 

    for img_path in img_paths:
        img_path=join(imgDir_path,img_path)
    	ind=int(re.findall(r"images/image(.+?).png",img_path)[0])
    	est_name='prediction/image%d.png_pose.npz' % ind
        est = np.load(join(data_dir, est_name))['pose']
        out_path =join(out_dir,'image%d_smpl.pkl' % ind) 
        if not exists(out_path):
            ...
            if not use_neutral:
                #gender = 'female' 
#if int(genders[ind]) == 0 else 'female'
                if gender == 'female':
                    model = model_female
                    if use_interpenetration:
...
if __name__ == '__main__':
    ...
    main(args.gender,args.base_dir, args.out_dir, use_interpenetration, args.n_betas,args.flength, args.side_view_thsh, args.gender_neutral, args.viz)
